...
sys.path.insert(0, str(BASE_DIR))  # Allow imports from backend/
...
def get_db():
    global db
    if db is None:
        from database.tidb_manager import get_database_manager
        # If using SQLite, place it in project root
        db_path = str(PROJECT_ROOT / "hackathons.db")
        # Note: the factory might not accept path arg if it calls TiDBManager.
        # But DatabaseManager takes a path.
        # Let's adjust usage or factory. 
        # The factory currently: return DatabaseManager('hackathons.db')
        # We need to ensure it uses the correct path if SQLite.
        db = get_database_manager() 
        if hasattr(db, 'db_path'): # It's SQLite
             db.db_path = db_path
             # Re-init conn if needed or just let it be if it does lazy load
    return db
