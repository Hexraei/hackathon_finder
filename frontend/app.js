...
async function initializeSourceFilter() {
    ...
    // Default to only these 4 sources on page load
    const defaultSources = ['Unstop', 'Devpost', 'Devfolio', 'DevDisplay'];
    state.selectedSources = new Set(defaultSources.filter(s => state.allSources.includes(s)));
    console.log('Initialized with default sources:', [...state.selectedSources]);
...
async function init() {
    ...
    showLoading(false);
    renderHackathons();
    updateResultsCount(); // Update count after initial load
}
...
function updateResultsCount() {
    if (elements.resultsCount) {
        elements.resultsCount.textContent = `${state.totalEvents} hackathons`;
    }
}
